...
def get_last_two_closed_15m_candles(symbol):
    """Returns the last two fully closed 15m candles as (open, close) tuples."""
    url = f"{BINANCE_FUTURES_BASE}/fapi/v1/klines"
...
def send_discord_alert(symbol, r5, candles):
    ...
    message = {
        "content": (
            f"**R5 Breakout — {symbol}**\n"
            f"Two consecutive green 15m candles closed above R5.\n"
            f"R5 level: `{r5:.6f}`\n"
...
def scan():
    ...
    for symbol in symbols:
        try:
            ...
            candles = get_last_two_closed_15m_candles(symbol)
            if not candles:
                continue
            (o1, c1), (o2, c2) = candles
            ...
            if candle1_green_above and candle2_green_above:
                print(f"[HIT] {symbol} — R5={r5:.6f}, candles closed at {c1:.6f} and {c2:.6f}")
